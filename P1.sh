...
detectar_conexiones_excesivas() {
    ...
    # Recorro cada línea de la lista de IPs y sus conexiones usando un bucle for
    for ip_por_ip in $ips; do
        ...
        # Verificar si contador_conexiones es un número
        if [[ ! "$contador_conexiones" =~ ^[0-9]+$ ]]; then
            echo "Error: El contador de conexiones no es un número válido."
            continue
        fi
