#!/bin/bash
# Habilitar la opción para detener el script en caso de error
set -e
...
detectar_patrones_maliciosos() {
    ...
    for patron in "${patrones_maliciosos[@]}"; do
        ...
        for ip in $ips_sospechosas; do
            ...
            for ip_bloqueada in $(cat "$ips_bloqueadas"); do
                if [ "$ip" = "$ip_bloqueada" ]; then
                    ip_encontrada=1
                    break
                fi
            done
...
detectar_conexiones_excesivas() {
    ...
    for ip_por_ip in $ips; do
        # Separo el número de conexiones y la IP
        contador_conexiones=$(echo "$ip_por_ip" | awk '{print $1}')
        ip=$(echo "$ip_por_ip" | awk '{print $2}')

        # Verificar si contador_conexiones es un número y si la IP no está vacía
        if [[ ! "$contador_conexiones" =~ ^[0-9]+$ ]] || [ -z "$ip" ]; then
            echo "Error: Datos no válidos en la línea: $ip_por_ip"
            continue
        fi

        # Si la IP supera el umbral de conexiones, la bloqueo
        if [ "$contador_conexiones" -gt "$conexiones" ]; then
            # Variable para verificar si la IP ya está bloqueada
            ip_encontrada=0
...
echo
echo "Proceso de mitigación completado."#!/bin/bash
# Habilitar la opción para detener el script en caso de error
set -e
